...
|---|---|---|---|---|
| Every page — header | `src/components/Header.astro` | mark + wordmark | 56/72 h + 27/32 h | reverse lockup, tagline stacked under wordmark, eager |
...
All placements go through `<Image>` with `densities={[2, 3]}` (except the watermark — a 6% opacity ghost needs no retina variants — and the two energy-core stills, soft glows served at one 512 px size). Astro generates per-site size/density variants and deduplicates identical outputs.
...
The home hero runs a muted, decorative background clip: an opening "ride" that plays once per browser session, then a seamless loop. Both are AI-generated (Higgsfield), graded and encoded by `scripts/make-ambient-video.mjs`, and shipped from `src/assets/video/`. Brand rules: `XTEND-AI-WEB.md` §4.1, §12.5, §13, §14.2.

### What ships

| File | Role | Size | Notes |
|---|---|---|---|
| `beneath-hero-d.mp4` | loop, landscape (`(min-width: 1024px)`) | 1.25 MB | 1600×900, H.264 High L4.0, CRF 23, 24 fps; a 1.2 s tail-into-head crossfade closes the loop |
| `beneath-hero-m.mp4` | loop, portrait | 0.71 MB | 540×960, L3.1, CRF 24; crop window at 80% of the master's width, on the lattice |
| `beneath-ride-d.mp4` | opening clip, landscape | 2.41 MB | 1600×900, L4.0, CRF 26; plays once, left as generated |
| `beneath-ride-m.mp4` | opening clip, portrait | 0.89 MB | 540×960, L3.1, CRF 27; crop window travels 0.8 → 0.5 → 0.8 across the clip, following the vanishing point |
| `beneath-hero-d.png` / `-m.png` | first frame of each loop rendition | 464 / 312 KB source | served as WebP (q70, ~22 / 15 KB) under the clip: the LCP element |

Budgets are enforced by the script (`budgetBytes` per rendition: 2.5 / 1.2 / 3.5 / 1.5 MB); an over-budget file is deleted and the run fails. The masters (1920×1080 HEVC Main 10, 24 fps, 10 s, as Higgsfield delivers them) live in gitignored `video-masters/`. A fresh clone can build the site but not regenerate the clips; that is intended, the outputs are committed and the masters are large.

### Components

`AmbientVideo.astro` (props `desktop`, `mobile`, `desktopStill`, `mobileStill`, `desktopMedia`, `intro`, `introMobile`, `startDelay`, `minWidth`) renders `.ambient` > `.ambient__media[aria-hidden]` > `<picture>` still + `<video data-role="loop">` + optional `<video data-role="intro">` + `.ambient__scrim`. The `<video>` elements carry **no `autoplay`, no `src`, no `poster`**. The script assigns `src` after `load` + `startDelay` + idle, and only if nothing says no: reduced motion, Save-Data, 2g/3g, `deviceMemory ≤ 2`, a stored "off" choice, a hidden tab, an off-screen box, a viewport under `minWidth`. `NotAllowedError` (iOS Low Power Mode) cancels the download and never retries. One media query (`desktopMedia`) picks both the still and the rendition, so they always match. The intro buffers the loop during its own last two seconds; `ended` (or `error`) hands off to the loop with no fade (`is-handoff`), and `sessionStorage['xt-ambient-intro']` skips the intro for the rest of the session. Every clip pauses when scrolled out of view or when the tab is hidden, and resumes after a bfcache restore (`pageshow`).
...
Inside `.hero__visual` (which keeps the entrance `rise`; two transform animations on one element fight), `.tess` reserves a square box (`width: var(--cube-size)`, `aspect-ratio: 1`, `pointer-events: none`) and holds `.tess__float` (the CSS float) with, back to front: `.tess__backing` (a CSS radial glow of `--xt-accent`, the wide haze the frames have around the object and a contrast plate), two optional core stills (`coreA` / `coreB` props; RGBA `<Image>`s, normal blending, counter-rotating and breathing in CSS), the canvas, and the DOM lockup (mark over wordmark), which is the no-JS fallback and the bitmap source and is hidden (`.has-logo`) once the canvas draws it. Every CSS loop is gated on `.is-live`, so without JavaScript the visitor gets a static glowing core and the lockup and nothing that would need a pause control. No `mix-blend-mode` anywhere: isolated, a blend mode blends against an empty backdrop; un-isolated it makes the compositor read back the video layer.
...
- **Guards**: reduced motion, Save-Data, 2g/3g and `deviceMemory ≤ 2` (the same predicate as the video, because the toggle's resume path is gated on it) draw one static frame in the hero pose and never start the loop; `data-ambient-off` (read from the attribute and `localStorage['xt-ambient']` at boot, then a `MutationObserver` on `<html>`) freezes the frame and pauses the CSS loops; the loop stops off screen (IntersectionObserver, threshold 0) and in hidden tabs, resumes after a bfcache restore. Adaptive tier: 2 → 1 → 0 when more than 15% of frame intervals exceed 34 ms over a rolling 3 s window (tier 1 drops the fine octaves and faces; tier 0 is a crisp pass at DPR 1 and 30 fps); 30 fps after a minute without interaction.
...
| `--cube-glow`, `--cube-face-alpha`, `--cube-backing`, `--cube-core-opacity` | 1, 0.03, 0.35, 0.85 | bloom, glass tint, CSS haze, core stills |
