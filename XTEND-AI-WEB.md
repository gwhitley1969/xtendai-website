...
Decisions locked by the owner: the ambient backdrop stays and became one continuous flight over a cloudless lattice (the clouds read as distracting; the opening ride was the part the owners loved, so it now loops); the lattice lights glow steadily (co-founder request); the phone becomes a tesseract with the mark + wordmark lockup at its core, reactive to the visitor; light pulses on the cube are off; the cube is a quarter larger than first built; every auto-playing motion carries the shared pause control.
...
**Implementation constraint:** the header renders the tagline at `white-space: nowrap`, 0.75rem on mobile (raised from 0.65rem with the 2026-08-28 banner header). The superseded tagline was 36 characters and fit at 375 px; this one is 34. Treat ~36 characters as the hard ceiling for any future change, and re-check at 375 px whenever it changes.
...
* Header content is the one exception: it spreads to 1440px so the logo and nav reach past the content edges on large screens (approved 2026-08-28, co-founder request; bar height 96px mobile / 132px desktop, always full-size, never condensing on scroll)
...
| File | Role | Size |
|---|---|---|
| `beneath-hero-d.mp4` / `-m.mp4` | the flight, looped, landscape / portrait (cloudless, the lattice glows steadily) | 3.02 MB / 1.18 MB |
| `beneath-hero-d.png` / `-m.png` | first frame of the loop, painted as the still under the clip (the LCP element) | served as WebP, largest variants 40 / 24 KB |
...
The hero tesseract's energy core is two RGBA glow stills in `src/assets/hero/` (`tesseract-core-a.png`, `-b.png`; Higgsfield GPT Image 2.5 with a transparent background, keyed and vignetted by `node scripts/make-tesseract-core.mjs` from sources in the gitignored `video-masters/stills/`). The cube itself is drawn live and ships no image; the lockup at its core uses the reverse mark and wordmark of §12.1a.
...
* Ambient video budgets, enforced by `scripts/make-ambient-video.mjs` (an over-budget rendition is deleted and the run fails): landscape loop ≤ 3.5 MB, portrait loop ≤ 1.5 MB (a flight needs more bits than a locked-off shot). Video is never part of first paint: it loads after `load` plus a delay, and only where nothing says no.
...
* Hero tesseract: the canvas never allocates per frame, drops no frames at 120 Hz on a laptop, and falls back through adaptive tiers (fewer bloom passes, lower DPR, 30 fps) on weak devices; each energy-core still ≤ 80 KB as served. With the cube in place the worst-case build measured 98 mobile / 100 desktop (2026-09-22), CLS 0, the still as the LCP element.
